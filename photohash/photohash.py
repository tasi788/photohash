...
def open_img(photo):
    ...
    elif type(photo) in [bytes, _io.BufferedReader]:
        return Image.open(photo)
    else:
...
def hex_to_hash(hexstr):
	...
	binary_array = '{:0>{width}b}'.format(
	    int(hexstr, 16), width=hash_size * hash_size)
	bit_rows = [binary_array[i:i+hash_size]
...
def average_hash(image, hash_size=8):
	...
	image = open_img(image).convert("L").resize((hash_size, hash_size), Image.ANTIALIAS)
...
def phash(image, hash_size=8, highfreq_factor=4):
	...
	image = open_img(image).convert("L").resize((img_size, img_size), Image.ANTIALIAS)
	pixels = numpy.asarray(image)
...
def dhash(image, hash_size=8):
	...
	image = open_img(image).convert("L").resize((hash_size + 1, hash_size), Image.ANTIALIAS)
	pixels = numpy.asarray(image)
...
def dhash_vertical(image, hash_size=8):
	...
	"""
	# resize(w, h), but numpy.array((h, w))
	image = open_img(image).convert("L").resize((hash_size, hash_size + 1), Image.ANTIALIAS)
	pixels = numpy.asarray(image)
...
def whash(image, hash_size = 8, image_scale = None, mode = 'haar', remove_max_haar_ll = True):
	...
	image = open_img(image).convert("L").resize((image_scale, image_scale), Image.ANTIALIAS)
